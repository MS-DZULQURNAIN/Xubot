...
__HELP__ = f"""
Perintah:
         <code>{PREFIX[0]}song</code> [music title]
Penjelasan:
           Untuk mendownload music yang diinginkan

Perintah:
         <code>{PREFIX[0]}vsong</code> [video title]
Penjelasan:
           Untuk mendownload video yang diinginkan
"""


@ubot.on_message(filters.me & filters.command("song", PREFIX))
async def _(client, message):
    if len(message.command) < 2:
        return await message.reply(f"<code>{message.text}</code> judul music")
    TM = await message.reply("<b>🔎 Pencarian...</b>")
    text = f"{message.text.split(None, 1)[1]}"
    x = await client.get_inline_bot_results("lybot", text)
    try:
        saved = await client.send_inline_bot_result(
            client.me.id, x.query_id, x.results[0].id
        )
    except:
        await message.reply("<b>❌ Music tidak ditemukan</b>")
        await TM.delete()
...
@ubot.on_message(filters.me & filters.command("vsong", PREFIX))
async def _(client, message):
    if len(message.command) < 2:
        return await message.reply(f"<code>{message.text}</code> judul video")
    TM = await message.reply("<b>🔎 Pencarian...</b>")
    text = f"{message.text.split(None, 1)[1]}"
    x = await client.get_inline_bot_results("vid", text)
    await client.unblock_user("@youtubednbot")
    try:
        await client.send_inline_bot_result(
            "@youtubednbot", x.query_id, x.results[0].id
        )
    except:
        await message.reply("<b>❌ Video tidak ditemukan</b>")
        await TM.delete()
    await asyncio.sleep(8)
    async for vsong in client.search_messages("@youtubednbot"):
        if vsong.video:
            await client.send_video(
                message.chat.id, vsong.video.file_id, reply_to_message_id=message.id
            )
            await TM.delete()
        else:
